...
| Files shows **OneDrive — error** / `notSupported` | Graph refused the source-drive read. If **Pre-flight is 4/4** (perms fine), the plan row appends the source user's **SharePoint plan** status: **"no SharePoint plan / not serviceable"** = the source account isn't licensed for OneDrive (common for a **cut-over user** whose E3 now lives on the target and whose source carries only a **cross-tenant license**) — there's nothing readable on the source. **"SharePoint plan Enabled"** = likely **multi-geo** (drive in another geo). If perms aren't 4/4, grant `Files.ReadWrite.All` + `Sites.ReadWrite.All` on the source app and consent. |
