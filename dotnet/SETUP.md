...
| Files shows **OneDrive — error** / `notSupported` | Graph refused the source-drive read. If **Pre-flight is 4/4** (perms fine), the plan row appends the source user's **SharePoint plan** status: **"no SharePoint plan / not serviceable"** = the source account isn't licensed for OneDrive (common for a **cut-over user** whose E3 now lives on the target and whose source carries only a **cross-tenant license**) — there's nothing readable on the source. **"SharePoint plan Enabled"** = **multi-geo**: the drive lives in another geo, so `/users/{id}/drive` won't serve it. **Fix:** the **Bulk OneDrive** workload handles multi-geo **automatically** (it falls back to each user's OneDrive site URL — no manual URLs). For a single user via **Files (OneDrive)**, put the user's **source OneDrive URL** in *Scope* and their **target OneDrive URL** in the *Target* field — the tool resolves the drive by site URL (geo-safe) and copies it. Get the URLs from the OneDrive admin / "Manage access". |
